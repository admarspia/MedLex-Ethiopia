...
function MedicineCard({ data }) {
    return (
        <Link to={`/medicines/${medicine.id}`} className="card animate-in" style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
            ...
            <h3 style={{ textTransform: 'uppercase', marginBottom: '0.5rem' }}>{data.medicine.generic_name}</h3>
            <p style={{ marginBottom: '1.5rem', fontSize: '0.9rem', color: 'var(--text-muted)', flex: 1 }}>{data.medicine.brand_name || 'Generic Formulation'}</p>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '1rem', borderTop: '1px solid rgba(0,0,0,0.05)' }}>
                <span style={{ fontSize: '0.8rem', fontWeight: 800 }}>
                    {data.pharmacies?.length || 0} Providers
                </span>
