...
class PharmacyAPI {
  ...
  public function addMedicine() {
    ...
    if (empty($genericName)) {
      return $this->response(422, "Medicine name required");
    }

    if ($count <= 0) {
      return $this->response(422, "Invalid count");
    }
    ...
    if (!isset($_FILES['image']) || $_FILES['image']['error'] !== 0) {
      return $this->response(422, "Image required");
    }

    $allowedTypes = ['image/jpeg', 'image/png'];

    if (!in_array($_FILES['image']['type'], $allowedTypes)) {
      return $this->response(422, "Only JPG/PNG allowed");
    }

    if ($_FILES['image']['size'] > 5 * 1024 * 1024) {
      return $this->response(422, "Image max size is 5MB");
    }
    ...
    try {
      ...
      if (!$medicineId){
        return $this->response(400, $medicine);
      }
  ...
  public function removeMedicine() {
    ...
    if (!isset($_SESSION["pharmacy_id"])) {
        ...
        if (!is_numeric($medicineId)) {
            return $this->response(422, "Invalid medicine ID");
        }
